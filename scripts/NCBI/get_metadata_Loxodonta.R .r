...
genus = "Loxodonta"
species_list <- c("Loxodonta africana", "Loxodonta cyclotis")
...
sra_result <- sra_result %>% filter(
  scientific_name %in% species_list & library_strategy == "WGS" & library_source == "GENOMIC" & library_selection == "RANDOM" & library_layout == "PAIRED")
...
selected_BioProject_accession <- c(rep("PRJNA247496", each=18),
                                   rep("PRJEB77639", each=908),
                                   rep("PRJNA840935", each=5), 
                                   rep("PRJNA761769", each=18),
                                   rep("PRJNA622303", each=10),
                                   rep("PRJNA216983", each=2))

organism_name <-                 c(rep("Loxodonta_africana", each=18),
                                   rep("Loxodonta_africana", each=674), rep("Loxodonta_cyclotis", each=234),
                                   rep("Loxodonta_africana", each=5), 
                                   rep("Loxodonta_africana", each=18),
                                   rep("Loxodonta_africana", each=10),
                                   rep("Loxodonta_africana", each=2))
...
for (sample in unique(data_stripped_pre$IND_ID)){
  ...
  for (accession in accessions){
    gbases <- gbases + filter(sra_result, run_accession == accession)$total_bases
...
data_stripped_pre_af <- subset(data_stripped_pre, grepl("^Loxodonta_africana", FOLDER))
data_stripped_pre_af$FOLDER <- "Loxodonta_africana"
data_stripped_pre_cy <- subset(data_stripped_pre, grepl("^Loxodonta_cyclotis", FOLDER))
data_stripped_pre_cy$FOLDER <- "Loxodonta_cyclotis"
data_stripped_pre <- bind_rows(data_stripped_pre_af,data_stripped_pre_cy)
...
data_stripped$REFERENCE_FOLDER[data_stripped$GENUS == "Loxodonta"] <- "Loxodonta_africana"
