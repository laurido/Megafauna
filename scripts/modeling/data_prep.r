...
#  group <- "Elephas_maximus"
#  ref_folder <- "Elephas_maximus"
...
#    mutate(time_ya = Generation * gen_time)
  ...
  df_ne <- df_smc  %>% select(time_ya, Ne) %>% 
    mutate(log_time = ifelse(time_ya != 0, log10(time_ya), NA),
                               log_ne = ifelse(Ne != 0, log10(Ne), NA),
                               time_kya = time_ya / 1000)
  ...
  df_ne <- df_ne %>%
    ...
    filter(time_kya <= end_time) %>%
    add_row(time_kya   = end_time, time_ya  = end_time * 1000)
  df_ne <- df_ne %>%
    arrange(time_kya) %>%
    mutate(window_end = lead(time_kya)) %>%
    mutate(window_end = lead(time_kya, default = end_time))
  # Plot GONE and SMC
  df_ne %>% ggplot(aes(x = log_time, y = Ne)) +
    geom_step() + scale_x_continuous(labels = function(x) parse(text = paste0("10^", x))) + xlab("Years ago")
...
#    geom_step() + scale_x_continuous(labels = function(x) parse(text = paste0("10^", x)))
  ...
  df_npp <- read.csv(paste0(path_prefix, "/megaFauna/sa_megafauna/data/bioclim/npp.csv")) %>%
    ...
    mutate(time_kya = abs(time_bp_ka))
  ...
  join_climate <- function(df_ne, df_climate, time_col, suffix) {
    ...
    # Ensure consistent time variable
    df_climate <- df_climate %>%
      mutate(
        time_kya = abs(.data[[time_col]]),
        bin_start = time_kya,
        bin_end = time_kya + 1
      )
    ...
    df_ne %>%
      ...
      mutate(
        ...
        !!paste0("mean_", suffix) := {
          ...
          overlaps <- pmax(
            0,
            pmin(end, df_climate$bin_end) -
              pmax(start, df_climate$bin_start)
          )
        ...
        !!paste0("var_", suffix) := {
          ...
          if (!any(valid)) {
            ...
            mu <- sum(values * w) / sum(w)
            sum(w * (values - mu)^2) / sum(w)
          }
        }
  ...
  df_joined <- df_ne %>%
    ...
    mutate(
      pn.range := {
        ...
        overlaps <- pmax(
          0,
          pmin(end, df_npp$time_kya + 1) -
            pmax(start, df_npp$time_kya)
        )
        
        valid <- overlaps > 0
        
        if (!any(valid)) NA_real_
        else sum(df_npp$pn.range[valid] * overlaps[valid]) / sum(overlaps[valid])
      },
      ...
      range.size := {
        ...
        if (!any(valid)) NA_real_
        else sum(df_npp$range.size[valid] * overlaps[valid]) / sum(overlaps[valid])
      },
      Binomial.1.2       = group) %>%
  ...
  df_joined <- df_joined %>%
    ...
    df_joined <- df_joined %>% rename(p_human = human_pressure)
