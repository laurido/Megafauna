...
group = "Loxodonta_cyclotis"
ref_folder <- "Loxodonta_africana"
...
df_ne_windows <- df_ne %>%
  arrange(time_kya) %>%
  mutate(window_end  = lead(time_kya),           # next timepoint = end of this window
    window_size = window_end - time_kya) %>%     # size of each window in kya
    filter(!is.na(window_end))               # remove last row which has no end
...
df_joined <- df_ne_collapsed %>%
  ...
  mutate(
    ...
    Binomial.1.2       = df_npp$Binomial.1.2[1]) %>%
  ungroup() %>%
  filter(time_kya <= min(end_time/1000, 800))#%>% select(-window_end)
