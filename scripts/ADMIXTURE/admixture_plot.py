...
for idx, k in enumerate(range(k_min, k_max + 1)):
    ...
    bar_width = 1.0  # <-- gap between bars (was 1.0)
    bottom = np.zeros(len(q))

    for pop_idx, pop in enumerate([f"pop{i}" for i in range(k)]):
        ax.bar(range(len(q)), q[pop], bottom=bottom,
               color=colors[pop_idx % len(colors)], width=bar_width, edgecolor="none")
        bottom += q[pop].values

    # Population boundary lines and sample counts
    pop_groups = q.groupby("dominant_pop", sort=True)
    boundaries = pop_groups.size().cumsum().values
    pop_counts = pop_groups.size().values
    pop_names  = list(pop_groups.groups.keys())

    # Draw boundary lines between populations
    for boundary in boundaries[:-1]:
        ax.axvline(boundary - 0.5, color="black", linewidth=0.8, linestyle="--")  # -0.5 to center on gap

    # Sample count labels centered under each population group
    prev = 0
    for name, count in zip(pop_names, pop_counts):
        midpoint = prev + count / 2 - 0.5
        ax.text(midpoint, -0.12, f"{name} (n={count})", ha="center", va="top",
                fontsize=7, transform=ax.get_xaxis_transform())
        prev += count
    
    ax.set_xlabel("")  # clear the xlabel

    title = f"K={k}" if k != best_k else f"K={k} ← best"
    ax.set_title(title, fontsize=9)
    ax.set_xlim(-0.5, len(q) - 0.5)  # <-- fixes the white bar and axis shift
    ax.set_ylim(0, 1)
