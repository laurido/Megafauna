...
sra_result <- sra_result %>% filter(
  any(startsWith(scientific_name, paste0(genus, " ", species))), 
...
selected_BioProject_accession <- c(rep("PRJNA767781", each=9),
                                   rep("PRJEB45263", each=9),
                                   "PRJNA399457") 

organism_name <-                 c(rep("Ceratotherium_simum_simum", each=9),
                                   rep("Ceratotherium_simum", each=9),
...
data_stripped_pre$FOLDER <- "Ceratotherium_simum"
...
data_stripped$REFERENCE_FOLDER[data_stripped$FOLDER == "Ceratotherium_simum"] <- "Ceratotherium_simum"
...
write.table(data_stripped, paste0(path_prefix, "/megaFauna/people/laurids/new_samples/samples_",genus,".txt"), quote = FALSE, sep = "\t", row.names = F)
